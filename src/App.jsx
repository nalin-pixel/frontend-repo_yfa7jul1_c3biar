...
function App() {
  ...
  return (
    <div className={`min-h-screen bg-white text-gray-900 dark:bg-gray-950 dark:text-white`}>
      {/* Top bar with brand, language and theme toggles */}
      <header className="sticky top-0 z-50 w-full border-b border-gray-200 bg-white/80 backdrop-blur dark:border-gray-800 dark:bg-gray-950/80">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
          <div className="flex items-center gap-2 font-semibold">
            <span className="inline-flex h-8 w-8 items-center justify-center rounded-md bg-emerald-500 text-gray-900"><Shield size={16} /></span>
            <span>{t.brand}</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="hidden sm:inline-flex items-center gap-2 rounded-md bg-emerald-500 px-3 py-2 text-sm font-semibold text-gray-900 shadow shadow-emerald-500/20 transition hover:bg-emerald-400"
            >
      ...
      {authOpen && (
        <AuthModal
          ...
          onSubmit={() => {
            // In a future step, wire to backend (OAuth2/JWT, WebRTC room). For now, just close.
            setAuthOpen(false);
            alert('This is a demo. Backend auth and meeting creation will be wired next.');
          }}
        />
      )}
    </div>
    ...
    <section className="bg-gray-50 py-16 dark:bg-gray-950">
      <div className="mx-auto max-w-6xl rounded-2xl border border-gray-200 bg-white px-8 py-10 text-center shadow-sm dark:border-gray-800 dark:bg-gray-900">
        ...
        <div className="mt-6 flex items-center justify-center gap-3">
          <button onClick={onCreate} className="rounded-lg bg-emerald-500 px-5 py-3 text-sm font-semibold text-gray-900 shadow-lg shadow-emerald-500/20 transition hover:bg-emerald-400">
            {t.ctaBlockPrimary}
          </button>
...
function AuthModal({ mode, t, onClose, onSubmit }) {
  return (
      ...
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl dark:border-gray-800 dark:bg-gray-900">
        ...
        >
          <div>
            ...
            <input type="email" required className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm outline-none ring-emerald-500 focus:ring-2 dark:border-gray-700 dark:bg-gray-950" />
          </div>
          <div>
            <label className="mb-1 block text-sm text-gray-700 dark:text-gray-300">{t.password}</label>
            <input type="password" required className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm outline-none ring-emerald-500 focus:ring-2 dark:border-gray-700 dark:bg-gray-950" />
          </div>
          <div>
            <label className="mb-1 block text-sm text-gray-700 dark:text-gray-300">{t.twofa}</label>
            <input type="text" inputMode="numeric" pattern="[0-9]*" maxLength={6} className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm outline-none ring-emerald-500 focus:ring-2 dark:border-gray-700 dark:bg-gray-950" />
          </div>
          <button type="submit" className="w-full rounded-lg bg-emerald-500 px-4 py-2 text-sm font-semibold text-gray-900 transition hover:bg-emerald-400">{t.continue}</button>
          <p className="text-center text-xs text-gray-500 dark:text-gray-400">E2EE, OAuth2/JWT e WebRTC saranno abilitati nelle prossime iterazioni.</p>
        </form>
      </div>
    </div>
  );
